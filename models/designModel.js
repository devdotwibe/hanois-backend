...
static async create(data) {
  ...
  // 🔥 Check duplicate QUALITY only
  if (quality !== undefined) {
    const existing = await pool.query(
      `SELECT id FROM design WHERE quality = $1 LIMIT 1`,
      [quality]
    );

    if (existing.rows.length > 0) {
      const error = new Error("Design with this Quality already exists");
      error.field = "quality";
      throw error;
    }
  }
...
static async updateById(id, data) {
  ...
  if (data.quality !== undefined) {
    const existing = await pool.query(
      `SELECT id FROM design WHERE quality = $1 AND id <> $2 LIMIT 1`,
      [data.quality, id]
    );
